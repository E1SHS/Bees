<template>
  <div class="container mt-5">
    <form @submit.prevent="handleSubmit" class="form-container mx-auto p-4 shadow-sm rounded bg-light">
      <h2 class="mb-4 text-center">Log Symptoms Form</h2>
    ...
    <div class="accordion mt-5" id="logSymptomsAccordion">
      <div v-for="(item, index) in dataList" :key="index" class="accordion-item">
        ...
        <div :id="'collapse' + index" class="accordion-collapse collapse" :aria-labelledby="'heading' + index" data-bs-parent="#logSymptomsAccordion">
...
const hive = ref('');
const notes = ref('');
const dataList = ref<LogSymptomsItem[]>([]);
const isLoading = ref(false);

const hiveOptions = ref([
  { value: 'Hive 1', label: 'Hive 1' },
  { value: 'Hive 2', label: 'Hive 2' },
  { value: 'Hive 3', label: 'Hive 3' },
  { value: 'Hive 4', label: 'Hive 4' },
  { value: 'Hive 5', label: 'Hive 5' }
]);

onMounted(async () => {
  await loadLogSymptomsRecords();
});

async function loadLogSymptomsRecords() {
  try {
    const response = await axios.get<LogSymptomsItem[]>('http://localhost:5001/auth/logsymptoms', { headers });
    dataList.value = response.data;
  } catch (error) {
    console.error('Failed to load log symptoms records:', error);
  } finally {
...
async function handleSubmit() {
  try {
    ...
    const response = await axios.post<LogSymptomsItem>('http://localhost:5001/auth/logsymptoms', {
      date: date.value,
    ...
    console.error('Failed to submit log symptoms form:', error);
  } finally {
    isLoading.value = false;
  }
}
